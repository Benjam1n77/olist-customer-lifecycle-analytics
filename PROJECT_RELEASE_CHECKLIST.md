...
- `README.md` 为英文首页，突出业务问题、技术方案、真实发现、最终 Dashboard 和复现方式。
- `README.zh-CN.md` 保留原有中文正文，包括项目架构、数据流程和用户分层三张真实项目图片；仅新增顶部语言切换与语言提示。
- 两份 README 顶部均提供 `English | 简体中文` 双向链接，并说明详细报告、Notebook 叙述与 Tableau 原始标注继续保留中文。
- 英文首页保留加权 M1 留存率 0.48%、90 天跨日二购与模型统计关联的口径边界；没有翻译或改写分析数据和最终截图。
...
### 首次发布前的本地复核（2026-08-27）

- 67 项自动化测试通过；展示 Notebook 的 9 个代码单元实际执行成功。
- 添加许可证后，检查 54 个本地 Markdown/Notebook 链接，未发现断链或链接到被忽略的文件。
- 完整简历排除并添加许可证后，首次发布前的可发布候选为 100 个文件；不包含 `outputs/local/`、旧工具或完整个人简历。当时再次检查大文件、14 份公开 CSV 表头、高置信度密钥格式，以及可发布 Markdown/LaTeX 中的邮箱和大陆手机号格式，均无异常。客户级展示仅限已审核说明页中的 6 条真实记录，不发布完整客户 CSV。
...
## 双语 README 更新（2026-08-27）

- 更新 `README.md`、新增 `README.zh-CN.md`，并同步本清单；未修改详细报告、Notebook、分析代码、数据或 Tableau 文件。
- 中文版去除新增的语言切换与提示后，与更新前 `3fe8bc8` 中的 README 正文逐字一致。
- 从现有 Cohort 明细重新核对加权 M1 留存率 0.48%（390 / 81,265；21 个成熟 Cohort），并核对三项二购模型结果的 OR、置信区间、p 值及样本量；英文版没有改动指标含义或结论边界。
- 本地 67 项测试通过；81 个 Markdown/Notebook 本地链接全部可解析且指向可发布文件，双向语言链接检查通过。
- 可发布文件增至 101 个，新增文件仅为中文 README；完整个人简历和本地产物仍被排除。英文完整重建说明注明数据库重建会清空目标表，本次未运行重建或重算生产分析。
