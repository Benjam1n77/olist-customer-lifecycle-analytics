...
本页展示 **{len(records)} 条真实客户级派生记录**，每类运营规则一条。购买金额、评分等来自项目实际处理结果；标签、建议动作和渠道来自已有 SQL 规则。没有虚构客户、随机造数或重新计算指标。
...
- 本地完整来源：`{source_label}`，本次源文件共 {source_rows:,} 条记录；完整 CSV 不随 GitHub 发布。
